...
<%@ page import="com.example.ParseFunction" %><%--
    ...
    <div id="accountdetails">
        ...
        <%for(int i = PaginationDao.paginationTransaction(len-1); i< PaginationDao.paginationTransaction(len); i++){%>
            <%String pair = (String) ActionsDao.list1.get(i);
                id=UserDao.getID((String)session.getAttribute("name"),(String) session.getAttribute("password"));
            Double value = (Double) ActionsDao.list2.get(i);
            BuyDao.selectquantity(pair,id);%>
        ...
        <tr>
            ...
            <%if(value>ParseFunction.currencypairs.get(pair)){%>
            <th><img id="downarrow" src="E:\workspace\Craiova-Internship-2017\CcyXcg\images\arrow_down.png"></th>
            <%}else if(value<ParseFunction.currencypairs.get(pair)){%>
            <th><img id="uparrow" src="E:\workspace\Craiova-Internship-2017\CcyXcg\images\arrow_up.png"></th>
            <%}else {%>
            <th>─</th>
            <%}%>
            <input type="hidden" name="valuetosell" value="<%=df.format(ParseFunction.currencypairs.get(pair))%>"><th><%=df.format(ParseFunction.currencypairs.get(pair))%></th>
            <th><%=df.format(((BuyDao.quantity*ParseFunction.currencypairs.get(pair))))%></th>
            <th><input type="text" name="quant" value=<%=BuyDao.quantity%>></th>
